...
# ------------------------------------------------------------------
# 1. Cấu hình thiết bị (GPU nếu có, fallback về CPU nếu không)
# ------------------------------------------------------------------
device = "cuda" if torch.cuda.is_available() else "cpu"
print(f"🖥️ Khởi tạo hệ thống nhúng Vector (QDRANT ENGINE - {device.upper()})")
if device == "cuda":
...
# ------------------------------------------------------------------
# 2. Tải mô hình lên đúng device
# ------------------------------------------------------------------
model, _, preprocess = open_clip.create_model_and_transforms(
    'ViT-H-14', pretrained='dfn5b', device=device
)
model.eval()

# (tuỳ chọn) chạy fp16 trên GPU để nhanh hơn và tiết kiệm VRAM
USE_FP16 = device == "cuda"
if USE_FP16:
    model = model.half()

# ------------------------------------------------------------------
# 3. Kết nối tới Qdrant Local
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 4. Trích xuất theo batch (batch lớn hơn vì chạy trên GPU)
# ------------------------------------------------------------------
...
for i in tqdm(range(0, len(image_paths), BATCH_SIZE), desc="Đẩy dữ liệu lên Qdrant"):
    ...
    with torch.no_grad():
        ...
        # QUAN TRỌNG: phải .cpu() trước khi .numpy() vì tensor đang ở GPU
        embeddings = image_features.float().cpu().numpy().tolist()
...
print("🎉 XONG! Dữ liệu ảnh đã được lưu trong Qdrant Local.")
print("   Xem tại: http://localhost:6333/dashboard")
